...
1.  Download the latest **[HardcoreWorldReset-2.2.0.jar](https://github.com/kvmhl/hardcore-world-reset/releases/latest)**.
...
The default seamless swap prepares the active and standby world sets before the
timer starts. Paper's asynchronous chunk API is used for the configured spawn
radius, and the timer waits until the active area is ready. The next standby
world is warmed after each swap so world generation is not part of the timed
run. Larger radii reduce first-visit chunk lag but increase preparation time
and disk usage.
...
prefix, for example `2.2.0` / `v2.2.0`. Minecraft compatibility is tracked
...
```
The artifact will be created in `target/HardcoreWorldReset-2.2.0.jar`.
