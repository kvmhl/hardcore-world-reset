...
1.  Download the latest **[HardcoreWorldReset-2.3.0.jar](https://github.com/kvmhl/hardcore-world-reset/releases/latest)**.
...
| `swap-method` | `"SEAMLESS"` | `SEAMLESS` (teleport) or `DISCONNECT` (kick players). |
...
The default seamless swap prepares the active world, the standby world, and
one additional buffer world before the timer starts. Paper's asynchronous
chunk API is used for the configured spawn radius, with one chunk request at a
time to avoid a generation burst. On reset, the next swap world is already
available; the following buffer is prepared before players are teleported and
before the new timer starts. World generation therefore does not run during
an active timed run. Larger radii reduce first-visit chunk lag but increase
preparation time and disk usage.
...
prefix, for example `2.3.0` / `v2.3.0`. Minecraft compatibility is tracked
...
```
The artifact will be created in `target/HardcoreWorldReset-2.3.0.jar`.
